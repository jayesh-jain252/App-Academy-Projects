...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === "GET" && req.url === "/dogs") {
      // Your code here
      let htmlDogs = fs.readFileSync("./views/dogs.html", "utf-8");
      const resBodyDogs = htmlDogs.replace(
        /#{dogsList}/g,
        `<li>${dogs[0]["name"]}</li>
          <li>${dogs[1]["name"]}</li>`
      );
    ...
    if (req.method === "GET" && req.url.startsWith("/dogs/")) {
      ...
      if (urlParts.length === 3) {
        ...
        // Your code here
        let htmlDog = fs.readFileSync("./views/dog-details.html", "utf-8");
        const resBodyDog = htmlDog
