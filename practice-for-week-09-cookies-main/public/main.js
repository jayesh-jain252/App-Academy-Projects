...
function storeTheme(themeName) {
  // Your code here
  return (document.cookie = `theme=${themeName}`);
}
...
function restoreTheme() {
  ...
  for (let i = 0; i < cookie.length; i++) {
    let string_cook = cookie[i];
    if (string_cook.includes("theme")) {
      let currTheme = string_cook.split("=")[1];
      if (currTheme !== "") {
...
function storeName(displayName) {
  // Your code here
  return (document.cookie = `name=${displayName}`);
}
...
function restoreName() {
  ...
  for (let i = 0; i < cookie.length; i++) {
    let string_cook = cookie[i];
    if (string_cook.includes("name")) {
      let currName = string_cook.split("=")[1];
      if (currName !== "") {
